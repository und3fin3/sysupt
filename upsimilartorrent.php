...
if (!isset($CURUSER) || $CURUSER["uploadpos"] == 'no' || !user_can_upload()) {
    ...
    tr ( $lang_edit ['row_check'], "<input type=\"checkbox\" name=\"visible\"" . ($row ["visible"] == "yes" ? " checked=\"checked\"" : "") . " value=\"1\" /> " . $lang_edit ['checkbox_visible'] . "&nbsp;&nbsp;&nbsp;" . (get_user_class () >= $beanonymous_class || get_user_class () >= $torrentmanage_class ? "<input type=\"checkbox\" name=\"uplver\"" . ($row ["anonymous"] == "yes" ? " checked=\"checked\"" : "") . " value=\"1\" />" . $lang_edit ['checkbox_anonymous_note'] . "&nbsp;&nbsp;&nbsp;" : "") . (get_user_class () >= $torrentmanage_class ? "<input type=\"checkbox\" name=\"banned\"" . (($row ["banned"] == "yes") ? " checked=\"checked\"" : "") . " value=\"yes\" /> " . $lang_edit ['checkbox_banned'] : "") . (get_user_class () >= UC_UPLOADER ? "<br />" . "<input type = \"checkbox\" name = \"exclusive\" " . ($row ["exclusive"] == "yes" ? " checked=\"checked\"" : "") . " value = \"yes\" />" . $lang_edit ['checkbox_exclusive'] . "&nbsp;&nbsp;&nbsp;" . "<input type = \"checkbox\" name = \"tjuptrip\"" .  ($row ["tjuptrip"] == "yes" ? " checked=\"checked\"" : "") . " value = \"yes\" />" . $lang_edit ['checkbox_tjuptrip'] : ""), 1 );
