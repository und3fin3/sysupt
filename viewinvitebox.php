    ...
    if ($_POST['setdealt']) {
        ...
        while ($arr = mysql_fetch_assoc($res)) {
            if ($arr ['dealt_by'] == 'no') {
                ...
                $email = $arr['email'];
                $email0 = htmlspecialchars(trim($email));
                invite($email0);
                print "邀请'" . $email0 . "'成功";
...
?>
    ...
    <table width="100%">
        ...
        <tr>
            <td class="text" valign="top">
                <div style="margin-left: 16pt;">1.点击右面的复选框，勾选要处理的申请；<br/>2.“设为已处理”将忽略此申请；“邀请”将向该申请邮箱发送邀请码（不会占用你的邀请名额）；尽量不要一次勾选多个申请同时邀请，以免其中一个邮箱有问题而影响其他邮箱。<br/>3.请认真审核，仔细处理。优先考虑网络、硬盘条件较好以及经验丰富的用户加入。<br/>
                </div>
            </td>
        </tr>
        </tbody>
	...
	<td class=\"rowfollow\"><input type=\"checkbox\" name=\"invitebox[]\" value=\"$Id\"></td>
        ...
        ?>
        <tr>
            <td class="colhead"><input class="btn" type="button" value="全选" onclick="this.value=check(form,'全选','全不选')">
            </td>
            <td class="colhead" colspan="12" align="right"><input type="submit" name="setdealt" value="忽略"/><input
                        type="submit" name="invite" value="邀请"/><input type="submit" name="delete" value="删除"/></td>
        </tr>
        </form></table>
<?php
echo $pagerbottom;
